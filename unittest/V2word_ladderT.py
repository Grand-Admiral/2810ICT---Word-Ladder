...
def find(word, words, seen, target, path, short):#word: 
  global maxMatch
  maxMatch = 0
  #build a list
  list = []
...
def main(fname, start, target, forbidInput, short, another):
  ...
  while True: # Try to open file
    ...
    while True: 
      ...
      if (start == target): #if both inputs are same, error and loop
          error = "Error. Start and target words cannot be the same. Try again."
          return error
